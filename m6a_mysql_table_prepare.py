...
try:
	...
	sys.exit('<m6a_table, m6adb e.g Homo_sapiens.GRCh38.95.utr_table.m6a.tmp2, RMVar_Human_basic_info_m6Am.txt>')

outfile = '.'.join(infile.split('.')[:-3]) + '.m6a' #strip('.tmp2') + '.phastConsElements'
...
for i in range(0, len(df)):          
	row = df.iloc[i,]                                                                                                  
	d.setdefault(row.rm_id, [row.modification_sample, row.modification_sample_information, row.pubmed_id, row.confidence_level, row.rbp_id])
...
with open(infile,'r') as f:
	for line in f:
		...
		if line[6] == '5_utr':
			dic_line.setdefault((line[0],line[3], line[4], line[5]))[0].append(line + d.get(line[10]))
			dic_line.setdefault((line[0],line[3], line[4], line[5]))[1].append(genomic_editing)
		else:
			dic_line.setdefault((line[0],line[3], line[4], line[5]))[2].append(line + d.get(line[10]))
			dic_line.setdefault((line[0],line[3], line[4], line[5]))[3].append(genomic_editing)

with open(outfile, 'w+') as g:
	g.write('{}\t{}\t{}\t{}\t{}\n'.format('Chrom','Gene_ID','Transcript_id','Strand','m6a_events'))
